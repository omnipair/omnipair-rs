...
pub fn max_borrowable_with_safety(
    ...
    let sqrt = v_over_r1.sqrt().unwrap_or(0); // result is scaled by 1e9

    // Approximate solution: Y ≈ V * (1 - sqrt(V / R1))
    let approx = (v * (NAD_U128 - sqrt)) / NAD_U128;

    // Newton-Raphson refinement
    // f(Y) = V * (1 - Y / R1)^2 - Y
    // f'(Y) = -2V * (1 - Y / R1) * (1 / R1) - 1
    let y = approx;

    let one_minus_y_r1 = NAD_U128.saturating_sub((y * NAD_U128) / debt_reserve as u128); // (1 - Y / R1)
    let f_y = (v * one_minus_y_r1.saturating_mul(one_minus_y_r1)) / (NAD_U128 * NAD_U128) - y;
    let f_prime = 0u128
