use crate::{
    ...
    utils::token::{
        require_supported_mint, token_program_for_mint, transfer_amounts_from_net,
...
impl<'info> CommonAdjustDebt<'info> {
    pub fn validate_repay(&self, args: &AdjustDebtArgs) -> Result<()> {
        ...
        let transfer_amount =
            transfer_amounts_from_net(&self.reserve_token_mint.to_account_info(), debt_to_repay)?;
    ...
    pub fn handle_repay(ctx: Context<Self>, args: AdjustDebtArgs) -> Result<()> {
        ...
        let transfer_amount =
            transfer_amounts_from_net(&reserve_token_mint.to_account_info(), debt_to_repay)?;
