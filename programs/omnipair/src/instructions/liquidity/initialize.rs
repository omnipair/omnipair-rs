...
use crate::utils::token::{
    create_token_account, require_supported_mint, token_mint_to, token_program_for_mint,
    transfer_amounts_from_gross, transfer_amounts_from_net, transfer_from_user_to_vault,
};
...
pub struct InitializeAndBootstrap<'info> {
    ...
    /// CHECK: created and initialized in handler with the token program matching token0_mint
    ...
    /// CHECK: created and initialized in handler with the token program matching token1_mint
...
impl<'info> InitializeAndBootstrap<'info> {
    ...
    pub fn handle_initialize(ctx: Context<Self>, args: InitializeAndBootstrapArgs) -> Result<()> {
        ...
        let token0_mint_info = ctx.accounts.token0_mint.to_account_info();
        let token1_mint_info = ctx.accounts.token1_mint.to_account_info();
        ...
        let token0_max = transfer_amounts_from_gross(&token0_mint_info, amount0_in)?;
        let token1_max = transfer_amounts_from_gross(&token1_mint_info, amount1_in)?;
        require!(
            token0_max.net > 0 && token1_max.net > 0,
            ErrorCode::AmountZero
        );
        let token0_contribution = transfer_amounts_from_net(&token0_mint_info, token0_max.net)?;
        let token1_contribution = transfer_amounts_from_net(&token1_mint_info, token1_max.net)?;
        require_gte!(
            amount0_in,
            token0_contribution.gross,
            ErrorCode::InsufficientAmount0In
        );
        require_gte!(
            amount1_in,
            token1_contribution.gross,
            ErrorCode::InsufficientAmount1In
        );
        ...
        let liquidity: u64 = (token0_contribution.net as u128)
            .checked_mul(token1_contribution.net as u128)
        ...
        pair.reserve0 = pair
            .reserve0
            .checked_add(token0_contribution.net)
            .ok_or(ErrorCode::ReserveOverflow)?;
        pair.reserve1 = pair
            .reserve1
            .checked_add(token1_contribution.net)
            .ok_or(ErrorCode::ReserveOverflow)?;
        ...
        emit_cpi!(MintEvent {
            metadata: EventMetadata::new(ctx.accounts.deployer.key(), pair.key()),
            amount0: token0_contribution.net,
            amount1: token1_contribution.net,
            liquidity: liquidity,
        });
