...
use super::common::{
    ...
    leverage_token_program_for_mint, quote_swap, require_initial_leverage_health,
    split_delegated_accounts, DelegatedCpiArgs, LEVERAGE_DELEGATE_INCREASE,
};
...
#[derive(AnchorSerialize, AnchorDeserialize, Clone)]
pub struct DelegatedIncreaseLeverageArgs {
    pub is_debt_token0: bool,
    pub debt_amount: u64,
    pub min_collateral_out: u64,
...
impl<'info> IncreaseLeverage<'info> {
    pub fn update_and_validate_increase(&mut self, args: &IncreaseLeverageArgs) -> Result<()> {
    ...
    pub fn update_and_validate_delegated_increase(
        &mut self,
        args: &DelegatedIncreaseLeverageArgs,
    ) -> Result<()> {
        self.update_and_validate_increase(&IncreaseLeverageArgs {
            is_debt_token0: args.is_debt_token0,
            debt_amount: args.debt_amount,
            min_collateral_out: args.min_collateral_out,
        })
    }

    pub fn handle_increase_leverage(
        ctx: Context<'_, '_, '_, 'info, Self>,
        args: IncreaseLeverageArgs,
    ) -> Result<()> {
        Self::execute(ctx, args, None, IncreaseMode::Owner)
    }

    pub fn handle_delegated_increase_leverage(
        ctx: Context<'_, '_, '_, 'info, Self>,
        args: DelegatedIncreaseLeverageArgs,
    ) -> Result<()> {
        Self::execute(
            ctx,
            IncreaseLeverageArgs {
                is_debt_token0: args.is_debt_token0,
                debt_amount: args.debt_amount,
                min_collateral_out: args.min_collateral_out,
            },
            Some(args.delegated),
            IncreaseMode::Delegate,
        )
    }
    ...
    fn execute(
        ...
        let delegated = match mode {
            IncreaseMode::Owner => DelegatedCpiArgs::default(),
            IncreaseMode::Delegate => delegated.ok_or(ErrorCode::InvalidLeverageDelegation)?,
        };
        ...
        let collateral_transfer = transfer_amounts_from_gross(
            &accounts.collateral_token_mint.to_account_info(),
            quote.amount_out,
        )?;
        require_gte!(
            collateral_transfer.net,
        ...
        let collateral_after = position
            .collateral_amount
            .checked_add(collateral_transfer.net)
            .ok_or(ErrorCode::Overflow)?;
        let debt_after = debt_before
            .checked_add(args.debt_amount)
        ...
        match mode {
            ...
            IncreaseMode::Delegate => {
                ...
                approved_for(delegation.approved_actions, LEVERAGE_DELEGATE_INCREASE)?;
        ...
        position.increase_debt(pair, args.debt_amount)?;
        ...
        emit!(SwapEvent {
            ...
            is_token0_in,
            amount_in: args.debt_amount,
            amount_out: collateral_transfer.net,
